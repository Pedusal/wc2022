...
for (i in 2:length(veikkaukset)) {
  final4taulu[i-1,1] <- as.data.frame(veikkaukset[i])[5,18] #haetaan mestari
}

## haetaan valieramaat
for (i in 2:length(veikkaukset)) {
  final4taulu[i-1,2:5] <- sort( # aakkostetaan
...
bp <- ggplot(final4taulu, aes(x=Mestari
                              #,y=
                              #,group=  
                                    )) +
  geom_line(aes(color=Mestari))+
  geom_point(aes(color=Mestari)) + 
