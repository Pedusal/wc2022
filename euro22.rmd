...
maaliporssi <- as.data.frame(oikearivi[,10:12])
...
```{r pisteet printtaus, echo=FALSE, warning=FALSE}
...
as.datatable(formattable(pistetaulu,
              list(
                ...
                area(col=3:length(pistetaulu[1,]) ) ~ color_tile(
...
as.datatable(formattable(pistetaulu,
              list(Nimi = formatter("span", style = ~ style(color = "black",font.weight = "bold")),
                Yhteensa = color_tile("white", "green"),
              area(col=3:length(pistetaulu[1,]) ) ~ color_tile("transparent", "lightskyblue")
...
```{r veikkaustaulu luonti, include=FALSE}
library(formattable)
library(dplyr)
#library(knitr)
