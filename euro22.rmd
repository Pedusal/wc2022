 ...
 color_tile_mean <- function (...) {
  formatter("span", style = function(x) {
    style(display = "block",
          padding = "0 4px", 
          `border-radius` = "4px", 
          `background-color` = ifelse(x %in% pudonneet$...19, "red", "white")) # Remember to change the colors!
  })}
...
as.datatable(
  formattable(final4taulu,
              list(
                ...
                #,Yhteensa = color_tile("white", "green")
                #,Maalit = color_tile("white", "red"),
                #,area(col=1:length(final4taulu[1,]) ) ~ color_tile(
                 # "transparent", "lightskyblue")
                ,area(col=1:length(final4taulu[1,]) ) ~ color_tile_mean()
              )
  )
  ,
            options = list(
...
```{r PIIRRA_bonusmaalintekijat, echo=FALSE}
...
as.datatable(
  formattable(bonus_tekijat,
              list(
                ...
                "Pelaaja" = formatter("span", style = x ~ style(color = ifelse(x%in%maaliporssi$...10 , "green", "black")))
